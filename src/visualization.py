...
def build_soft_target_inspector_html(image_b64: str, logits: list[float], class_names: list[str], title: str, initial_temperature: float = 4.0) -> str:
    ...
    return f"""
<style>
  :root {{ --primary: #06b6d4; --accent: #ff6b6b; --accent2: #7c3aed; --card-grad: linear-gradient(135deg, #ffffff 0%, #f0fbff 100%); --muted: #475569; }}
  .kd-card {{ border-radius: 18px; padding: 20px; background: var(--card-grad); box-shadow: 0 20px 50px rgba(6, 182, 212, 0.08); border-left: 6px solid rgba(6,182,212,0.18); transition: transform 240ms ease; }}
  .kd-card:hover {{ transform: translateY(-6px); box-shadow: 0 28px 70px rgba(6,182,212,0.12); }}
  .kd-header {{ display: flex; gap: 24px; justify-content: space-between; align-items: center; flex-wrap: wrap; margin-bottom: 18px; }}
  .kd-header h3, .kd-card h3 {{ margin: 0; color: var(--primary); font-weight: 700; letter-spacing: -0.2px; }}
  .kd-header p, .kd-card p {{ margin: 6px 0 0 0; color: var(--muted); }}
  .temp-wrap {{ min-width: 240px; }}
  .temp-wrap label {{ display: block; margin-bottom: 8px; font-weight: 700; color: #022c43; }}
  .temp-wrap input[type=range] {{ width: 100%; accent-color: var(--primary); }}
  .kd-body {{ display: grid; grid-template-columns: 160px 1fr; gap: 24px; align-items: start; }}
  .preview {{ width: 160px; height: 160px; object-fit: cover; border-radius: 16px; border: 1px solid rgba(2,44,67,0.06); background: linear-gradient(180deg,#ffffff,#f7ffff); box-shadow: 0 8px 24px rgba(2,44,67,0.04); }}
  .chart {{ display: grid; gap: 10px; }}
  .bar-row {{ display: grid; grid-template-columns: 92px 1fr 52px; gap: 12px; align-items: center; padding: 6px 2px; border-radius: 10px; transition: background 180ms ease, transform 180ms ease; }}
  .bar-row:hover {{ background: rgba(6,182,212,0.03); transform: translateY(-2px); }}
  .bar-label {{ font-size: 0.9rem; color: #022c43; font-weight: 600; }}
  .bar-track {{ height: 14px; border-radius: 999px; background: rgba(2,44,67,0.06); overflow: hidden; }}
  .bar-fill {{ height: 100%; border-radius: inherit; background: linear-gradient(90deg, var(--accent) 0%, var(--primary) 60%, var(--accent2) 100%); transition: width 360ms cubic-bezier(.2,.8,.2,1); box-shadow: inset 0 -2px 6px rgba(0,0,0,0.06); }}
  .bar-value {{ font-variant-numeric: tabular-nums; text-align: right; color: #022c43; font-weight: 700; }}
  .examples-grid {{ display: grid; gap: 16px; }}
  .example-card {{ display: grid; grid-template-columns: 120px 1fr; gap: 16px; padding: 14px; border-radius: 14px; background: linear-gradient(180deg,#ffffff,#fbfdff); box-shadow: 0 8px 28px rgba(124,58,237,0.06); border: 1px solid rgba(124,58,237,0.06); transition: transform 220ms ease, box-shadow 220ms ease; }}
  .example-card:hover {{ transform: translateY(-6px); box-shadow: 0 18px 48px rgba(124,58,237,0.12); }}
  .example-card img {{ width: 120px; height: 120px; object-fit: cover; border-radius: 12px; box-shadow: 0 6px 18px rgba(2,44,67,0.06); }}
  .compare-table {{ width: 100%; border-collapse: collapse; }}
  .compare-table th, .compare-table td {{ padding: 12px 14px; border-bottom: 1px solid rgba(2,44,67,0.06); text-align: left; }}
  .compare-table thead th {{ background: linear-gradient(90deg, rgba(6,182,212,0.06), rgba(124,58,237,0.04)); color: #022c43; font-weight: 700; }}
</style>
<div data-testid="soft-target-inspector" class="kd-card">
  <div class="kd-header">
    ...
    <div class="temp-wrap">
      <label for="temp-slider">Temperature: <span id="temp-value" data-testid="temp-value">{initial_temperature:.1f}</span></label>
...
def build_distillation_curve_html(histories: dict) -> str:
    ...
    for key, label, color in [
      ("hard_only", "Hard labels only (alpha=1.0)", "#ff6b6b"),
      ("soft_only", "Soft targets only (alpha=0.0)", "#06b6d4"),
      ("best_alpha", "Best alpha", "#7c3aed"),
    ]:
